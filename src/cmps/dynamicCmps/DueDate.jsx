...
export function DueDate({ board, newOnSaveBoard, info, setIsTaskFocus, task }) {
    ...
    const handleDatePick = (date) => {
        ...
        if (startDate) {
            newOnSaveBoard({ type: 'task', board, taskId: task.id, key: "date", value: startDate })
        }
    }
        ...
        >
            {info ?
                <div className="inner-container flex align-center justify-center">
                    ...
                    <AiOutlineClose className="remove-date" onClick={() => newOnSaveBoard({ type: 'task', board, taskId: task.id, key: "date", value: null })} />
                </div>
                :
